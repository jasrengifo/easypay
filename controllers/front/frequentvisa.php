       ...
       //Validar si todos los articulos son de suscripcion    
        ...
        if($prod_ss>0){
            ...
            if(Configuration::get('EASYPAY_AUTORIZAR_PAGOS')==1){
                ...
                $new_trans = "INSERT INTO "._DB_PREFIX_."ep_frequent_transactions (id_user, id_pagamento, tipo_pagamento, autorizado, valor) VALUES ('".$this->context->customer->id."', '".$_POST['id_payment']."', 'frequent_visa', 2, ".round((float) $this->context->cart->getOrderTotal(true, Cart::BOTH), 2).");";

                Db::getInstance()->execute($new_trans);

            }else{
                if(!isset($nome_mp)){
                    $sql = "INSERT INTO "._DB_PREFIX_."ep_requests (status, id_ep_request, method_type, method_status, method_entity, method_reference, customer_easypay, id_cart, first_date, updated, modo_de_pago, nombre_de_pago, id_user) VALUES ('ok', '".$_POST['id_payment']."', 'cc', 'pending', '', '', '".$this->context->customer->id."', ".$cart_number->cart->id.", NOW(), NOW(), '".$modo_de_pago."', '', ".$this->context->customer->id.")";
                }else{
                ...
                $new_trans = "INSERT INTO "._DB_PREFIX_."ep_frequent_transactions (id_user, id_pagamento, tipo_pagamento, autorizado, valor, ativado, id_cart) VALUES ('".$this->context->customer->id."', '".$_POST['id_payment']."', 'frequent_visa', 0, ".round((float) $this->context->cart->getOrderTotal(true, Cart::BOTH), 2).", 1, $cart->id);";
